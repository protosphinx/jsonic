...
fn main() {
    ...
    println!("▸ Network node initialized (Solstice every {} heartbeats)", node.solstice_interval);
    println!();
    ...
    println!("  DAO 1: {} [{}] id={}…", acme.dao.profile.name, acme.dao.profile.sector, &acme.dao.id[..12]);
    println!("  DAO 2: {} [{}] id={}…", globex.dao.profile.name, globex.dao.profile.sector, &globex.dao.id[..12]);
    println!("  DAO 3: {} [{}] id={}…", initech.dao.profile.name, initech.dao.profile.sector, &initech.dao.id[..12]);
    println!();
    ...
    println!("  📄 Invoice: {} → {} | $50,000 | '{}'", acme.dao.profile.name, globex.dao.profile.name, inv1.description);
    let inv1_id = inv1.id.clone();
    node.submit_transaction(inv1);

    // Globex pays Acme's invoice
    let pay1 = globex.create_payment(&acme_id, 50_000.0, "USD", &inv1_id, "Payment for tech consulting Q1");
    println!("  💰 Payment: {} → {} | $50,000 | settles invoice", globex.dao.profile.name, acme.dao.profile.name);
    ...
    println!("  📄 Invoice: {} → {} | $30,000 | '{}'", initech.dao.profile.name, acme.dao.profile.name, inv2.description);
    let inv2_id = inv2.id.clone();
    node.submit_transaction(inv2);

    // Acme pays Initech
    let pay2 = acme.create_payment(&initech_id, 30_000.0, "USD", &inv2_id, "Payment for strategy consulting");
    println!("  💰 Payment: {} → {} | $30,000 | settles invoice", acme.dao.profile.name, initech.dao.profile.name);
    ...
    println!("  📄 Invoice: {} → {} | €75,000 | '{}'", globex.dao.profile.name, initech.dao.profile.name, inv3.description);
    ...
    for tick in 1..=node.solstice_interval {
        match node.heartbeat() {
            Some(distributions) => {
                println!("  ♥ Heartbeat #{}: ★ SOLSTICE — Main-chain block #{} created", tick, node.main_chain.height());
                println!();
    ...
    println!("  Total token supply: {:.4}", node.main_chain.total_token_supply);
    ...
    for (id, chain) in &node.side_chains {
        ...
        println!("    Accounts receivable:  ${:.2}", balance.accounts_receivable);
